  ...
  return (
    <FormFieldShell id={field.id} label={field.label} description={field.description} error={message} required={field.required}>
      <Controller name={field.name} control={control} render={({ field: controller }) => {
        ...
        return (
          <button type="button" role="switch" aria-checked={checked} disabled={field.disabled}
            ...
            <span className={`relative h-6 w-11 rounded-full transition-all duration-300 ${checked ? "bg-[var(--tenant-primary)]" : "bg-slate-300 dark:bg-slate-700"}`}>
              <span className={`absolute top-1 size-4 rounded-full bg-white shadow-sm transition-transform duration-300 ${checked ? "translate-x-6" : "translate-x-1"}`} />
            </span>
