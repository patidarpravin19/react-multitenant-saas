...
export function NativeInputField({ field, register, errors }: DynamicFieldProps) {
  const supported = ["password","number","tel","url","date","time","datetime","month","week","color","range"];
  ...
  const typeMap: Record<string, InputHTMLAttributes<HTMLInputElement>["type"]> = {
    password:"password", number:"number", tel:"tel", url:"url", date:"date", time:"time",
    datetime:"datetime-local", month:"month", week:"week", color:"color", range:"range"
  };
  const attrs: InputHTMLAttributes<HTMLInputElement> = {
    id: field.id, type: typeMap[field.type], disabled: field.disabled,
  ...
  return (
    <FormFieldShell id={field.id} label={field.label} description={field.description} error={message} required={field.required}>
      <input {...attrs} {...register(field.name, { valueAsNumber: field.type === "number" || field.type === "range" })} />
    </FormFieldShell>
  );
}
