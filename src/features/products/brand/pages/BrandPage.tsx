...
const initialBrands: Brand[] = [
  {
    ...
    name: "Samsung",
    code: "SAM",
    description: "Mobile and electronics brand",
    isActive: true,
    isDelete: false,
  },
];
...
export function BrandPage() {
  ...
  const createBrand = async (values: Record<string, unknown>) => {
    ...
    const brand: Brand = {
      ...
      vendorId,
      vendorName: vendors.find((vendor) => vendor.id === vendorId)?.name,
  ...
  return (
    <ProductCrudLayout
      ...
      form={<DynamicForm title="" fields={fields} onSubmit={createBrand} />}
    >
      <DynamicGrid
        title="Brand List"
        columns={brandColumns}
        data={brands.filter((brand) => !brand.isDelete)}
        mode="client"
