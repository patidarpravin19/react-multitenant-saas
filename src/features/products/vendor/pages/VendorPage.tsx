...
const initialVendors: Vendor[] = [
  {
    ...
    description: "Primary Samsung distributor",
...
export function VendorPage() {
  ...
  const createVendor = async (values: Record<string, unknown>) => {
    const vendor: Vendor = {
  ...
  const updateVendor = async ({
    ...
    notifications.success("Vendor updated", "Vendor was updated successfully.");
  };
  ...
  const deleteVendor = async (vendor: Vendor) => {
    ...
    notifications.success("Vendor deleted", `${vendor.name} has been deleted.`);
  };

  const visibleVendors = vendors.filter((item) => !item.isDelete);
  ...
  return (
    ...
    >
      <DynamicGrid
        ...
        inlineEdit={{
          enabled: true,
          editableFields: ["name", "code", "mobile", "email", "address"],
          validate: ({ values }) => {
        ...
        onView={(vendor) => {
          notifications.info(vendor.name, `${vendor.code} • ${vendor.email}`);
        }}
