...
export function VendorFormPage({ mode }: { mode: "create" | "edit" }) {
  ...
  const loadVendor = useCallback(async () => {
    ...
    try {
      setLoadError(null);
      setVendor(await vendorApi.getById(id));
    } catch (reason) {
      setLoadError(
        reason instanceof Error ? reason.message : "Unable to load vendor.",
      );
    }
  }, [id, mode]);
  useEffect(() => {
  ...
  return (
    <DynamicForm
      title={mode === "create" ? "Add Vendor" : "Update Vendor"}
      description={
        mode === "create"
          ? "Create a new product vendor."
          : "Change the vendor details."
      }
      fields={vendorFormConfig}
      initialValues={vendor ?? undefined}
      submitLabel={mode === "create" ? "Create Vendor" : "Save Changes"}
      onCancel={() => navigate("/products/vendors/list")}
      onSubmit={async (values) => {
        const saved =
          mode === "create"
            ? await vendorApi.create(toInput(values))
            : await vendorApi.update(id!, toInput(values));
