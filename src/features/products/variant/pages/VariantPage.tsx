...
const initialVariants: ProductVariant[] = [
  {
    id: "variant-s25-128",
    productModelId: "model-s25",
    productModelName: "Galaxy S25 FE",
    name: "8 GB + 128 GB",
    code: "8-128",
    ram: "8 GB",
    rom: "128 GB",
    processor: "Snapdragon 8 Gen 3",
    description: "Base storage variant",
    isActive: true,
    isDelete: false,
  },
];
...
export function VariantPage() {
  ...
  const createVariant = async (values: Record<string, unknown>) => {
    ...
    const variant: ProductVariant = {
      ...
      productModelId,
      productModelName: models.find((model) => model.id === productModelId)
        ?.name,
      name: String(values.name ?? ""),
      code: String(values.code ?? ""),
      ram: String(values.ram ?? ""),
      rom: String(values.rom ?? ""),
      processor: String(values.processor ?? ""),
    ...
    notifications.success(
      "Variant created",
      `${variant.name} was created successfully.`,
    );
  };
  return (
    <ProductCrudLayout
      ...
      form={<DynamicForm title="" fields={fields} onSubmit={createVariant} />}
    >
      <DynamicGrid
        title="Variant List"
