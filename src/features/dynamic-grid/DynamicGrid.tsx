...
export function DynamicGrid<TData>({
  ...
  // Stop an in-flight search immediately; the next request starts once typing settles.
  useEffect(() => {
            ...
            {rows.map((row, rowIndex) => {
              ...
              return (
                <tr
                  ...
                  className={`transition-all duration-300 ${selected ? "bg-[var(--tenant-secondary)]/70" : "bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800/60"}`}
                >
                  {selectable ? (
                    <td
                      ...
                      <div className="inline-flex items-center gap-1">
                        {rowActions.filter((action) => !action.hidden?.(row)).map((action) => {
                          ...
                          return <button
                            ...
                            className={`cursor-pointer rounded-md p-2 transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${action.variant === "danger" ? "text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40" : "text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"}`}
                          ><Icon size={16} /></button>;
                        })}
                      </div>
                    </td>
